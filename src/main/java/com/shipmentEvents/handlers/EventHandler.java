...
public class EventHandler implements RequestHandler<ScheduledEvent, String> {
  ...
  /**
   * Shipment events for a carrier are uploaded to separate S3 buckets based on the source of
   * events. E.g., events originating from the hand-held scanner are stored in a separate bucket
   * than the ones from mobile App. The Lambda processes events from multiple sources and updates
   * the latest status of the package in a summary S3 bucket every 15 minutes.
   *
   * <p>The events are stored in following format: - Each status update is a file, where the name of
   * the file is tracking number + random id. - Each file has status and time-stamp as the first 2
   * lines respectively. - The time at which the file is stored in S3 is not an indication of the
   * time-stamp of the event. - Once the status is marked as DELIVERED, we can stop tracking the
   * package.
   *
   * <p>A Sample files looks as below: FILE-NAME->
   * '8787323232232332--55322798-dd29-4a04-97f4-93e18feed554' >status:IN TRANSIT >timestamp:
   * 1573410202 >Other fields like...tracking history and address
   */
  public String handleRequest(ScheduledEvent scheduledEvent, Context context) {
    ...
    try {
      ...
      logger.log(
          String.format(
              "Failed to process shipment Updates in %s due to %s",
              scheduledEvent.getAccount(), ex.getMessage()));
  ...
  public ShopifyShop connectToShopify(String subdomain) {
    ...
    final ShopifySdk shopifySdk =
        ShopifySdk.newBuilder().withSubdomain(subdomain).withAccessToken(token).build();
  ...
  private void processShipmentUpdates(final LambdaLogger logger) throws InterruptedException {
    ...
    final Map<String, Pair<Long, String>> latestStatusForTrackingNumber =
        new HashMap<String, Pair<Long, String>>();
    final Map<String, List<KeyVersion>> filesToDelete =
        new HashMap<String, List<DeleteObjectsRequest.KeyVersion>>();
    for (final String bucketName : bucketsToProcess) {
      final List<KeyVersion> filesProcessed =
          processEventsInBucket(bucketName, logger, latestStatusForTrackingNumber);
    ...
    EventHandler.getS3Client()
        .putObject(
            Constants.SUMMARY_BUCKET, summaryUpdateName, latestStatusForTrackingNumber.toString());
  ...
  private List<KeyVersion> processEventsInBucket(
      String bucketName,
      LambdaLogger logger,
      Map<String, Pair<Long, String>> latestStatusForTrackingNumber) {
    final AmazonS3 s3Client = EventHandler.getS3Client();
  ...
  private void deleteProcessedFiles(Map<String, List<KeyVersion>> filesToDelete) {
    ...
    for (Entry<String, List<KeyVersion>> entry : filesToDelete.entrySet()) {
      final DeleteObjectsRequest deleteRequest =
          new DeleteObjectsRequest(entry.getKey()).withKeys(entry.getValue()).withQuiet(false);
